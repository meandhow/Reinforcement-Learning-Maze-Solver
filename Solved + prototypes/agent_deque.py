...
class ReplayBuffer:
    ...
    def append_transition(self, transition):
        ...
        if len(self.replay_buffer) == 0:
            transition_l = list(transition)
            transition_l.append(self.counter)
            transition_l.append(1)
            transition_t = tuple(transition_l)
            self.replay_buffer.append(transition_t)
        else:
    ...
    def sort_buffer(self,losses_list):
        ...
        for i in range(len(self.replay_buffer))
            self.replay_buffer[i] = list(self.replay_buffer[i])
            
            for index, loss_act in losses_list:
                if idx == index:
                    loss = loss_act
                    break
               

            print(s,a,r,s_prime,idx,loss)
...
class Agent:
    ...
    def set_next_state_and_distance(self, next_state, distance_to_goal):
        ...
        if not self.training :
            ...
            if buffer_size >= 20:
                minibatch = self.buffer.rand_mini_batch(int(buffer_size / 10)) #+ int(self.num_steps_taken/100)
